...
class Wire
  ...
  def initialize(instructions, id)
    ...
    instructions.split(',').map do |instruction|
      ...
      when /^(R|L|U|D)(.*)/
        ...
        raise "Unkown instruction: #{instruction}"
      end
    end
  end
  ...
  def each_segments
    ...
    new_point = []
    smallest_cost = 0
    each_parts do |direction, steps|
      case direction
      when 'U'
        new_point = [x1, y1 + steps] 
      when 'D'
        new_point = [x1, y1 - steps] 
      when 'L'
        new_point = [x1 - steps, y1] 
      when 'R'
        new_point = [x1 + steps, y1] 
      end
      cost += steps
      smallest_cost = cost
      if @costs.key?(new_point)
        #puts "Multiple position for #{new_point}"
        smallest_cost = @costs[new_point]
      end
      @costs[new_point] = cost
      yield [[[x1,y1], new_point], smallest_cost]
      x1, y1 = new_point
    end
  end
  ...
  def intersects(other_wire)
    intersects = []
    each_segments do |segA, costA|
      other_wire.each_segments do |segB, costB|
        intersect = intersection?(segA, segB)
        if intersect && intersect != [0,0]
          total_cost = costA + costB - distance(intersect, segA[1])  - distance(intersect, segB[1])
          puts "Cost is #{costA} #{costB} => #{total_cost}"
          intersects << [intersect, total_cost]
        end
      end
    end
    intersects
...
wire1 = Wire.new(input[0], 1)
wire2 = Wire.new(input[1], 2)

def intersection?(segA, segB)
  lowest_pointA = [segA[0][1], segA[1][1]].min
  highest_pointA = [segA[0][1], segA[1][1]].max

  lowest_pointB = [segB[0][1], segB[1][1]].min
  highest_pointB = [segB[0][1], segB[1][1]].max

  return false if lowest_pointA > highest_pointB || highest_pointA < lowest_pointB
  return false if lowest_pointB > highest_pointA || highest_pointB < lowest_pointA

  lefty_pointA = [segA[0][0], segA[1][0]].min
  righty_pointA = [segA[0][0], segA[1][0]].max

  lefty_pointB = [segB[0][0], segB[1][0]].min
  righty_pointB = [segB[0][0], segB[1][0]].max

  return false if lefty_pointA > righty_pointB || righty_pointA < lefty_pointB
  return false if lefty_pointB > righty_pointA || righty_pointB < lefty_pointA

  all_pointsA = if lowest_pointA == highest_pointA
                  (lefty_pointA..righty_pointA).map { |x| [x, lowest_pointA] }
                else
                  (lowest_pointA..highest_pointA).map { |x| [lefty_pointA, x] }
                end
  all_pointsB = if lowest_pointB == highest_pointB
                  (lefty_pointB..righty_pointB).map { |x| [x, lowest_pointB] }
                else
                  (lowest_pointB..highest_pointB).map { |x| [lefty_pointB, x] }
                end
  common = all_pointsA & all_pointsB
  raise "Many intersect????" if common.size > 1
  raise "No intersect????" if common.empty?
  common.first
end

i= 0
wire1.each_segments { |el| i+=1}
puts i

intersects = []
wire1.intersects(wire2).each do |point, cost|
  intersects << [point, cost]
end
puts intersects.map { |point, cost| point[0].abs + point[1].abs }.min

puts intersects.map { |point, cost| cost }.min
