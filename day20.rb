...
def outer_to_inner_distances(maze, portals)
  ...
  portals.select { |name, coords| coords.size > 1 }.each do |name, coords|
    ...
    meta_distances[["inner_#{name}", 'ZZ']] = distance_from_inner[portals['ZZ'].first]
  end
...
def update_distances_graph(graph, starting_point)
  ...
  while visited.any? { |_,v| !v }
    ...
    neighbours_graph[point].each do |candidate|
      ...
      if new_inception < 0
        debug "  Skipping #{candidate} because it does not exist at this inception level"
        next
      end
      next if visited[candidate + 'x' * new_inception]
      current_dist = distances[candidate]
      d = graph[[point,candidate]] || graph[[candidate,point]]
      best_dist = [current_dist, distances[full_point_name] + d].compact.min
      ...
      debug "  Adding #{candidate + 'x' * (new_inception)} as non visited (unless it was already visited)" unless visited.key?(candidate + 'x' * (new_inception))
